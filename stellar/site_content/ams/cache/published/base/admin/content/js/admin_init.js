...
$(function () {
	...
	if($("#create_new_menu_item").length){
		function new_menu_item(trigerObj,callbacks){
			...
			if($('#new_menu_item').length==0)$('body #staging').append('<div id="new_menu_item"></div>');
			$('#new_menu_item').load('/post/generate_stub.castle',
				{
			...
			function(){
				$( "#new_menu_item" ).dialog({
					...
					open:function(){
						...
						$('.tabs').tabs({width:450});
						$('#new_menu_item [type="submit"]').on("click",function(e){
					...
					close:function(){ $("#new_menu_item").remove(); $('body').css('overflow','auto'); },
					buttons: {
						"Close": function() {
