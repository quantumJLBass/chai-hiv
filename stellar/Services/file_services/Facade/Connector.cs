            ...
            switch (cmdName) {
                case "open":
                    if (!string.IsNullOrEmpty(parameters["init"]) && parameters["init"] == "1") {
                        return _driver.Init(target, posting_json_obj);
                    } else {
                        if (string.IsNullOrEmpty(target))
                            return Error.MissedParameter(cmdName);
                        return _driver.Open(target, !string.IsNullOrEmpty(parameters["tree"]) && parameters["tree"] == "1", posting_json_obj);
                    }
                case "tree":
                    if (string.IsNullOrEmpty(target))
                        return Error.MissedParameter(cmdName);
                ...
                case "extract": {
                        ...
                        return _driver.Extract(target, targets);
                    }
